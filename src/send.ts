...
export async function sendNotification(input: AuthorizedConfig, notification: NotificationInput, inputOptions: SendOptions = {}): Promise<SubmitResult> {
  ...
  const uploads = [...files, ...(image ? [image] : []), ...(icon ? [icon] : [])].map(file => {
    const meta = validateAttachmentMetadata(file); validateAttachmentData(file.data);
